...
def train(args, model, criterion, postprocessors, device):
    ...
    if not resume_checkpoint and os.path.exists(model_save_path):
        ...
        with open(os.path.join(output_directory, 'log.txt'), 'a') as f:
            f.write("*** WARNING: Output model path exists but is not being used to resume training; training will overwrite it.\n")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with open(os.path.join(output_directory, 'log.txt'), 'a') as f:
            f.write(f"Epoch completed in {datetime.now() - epoch_timing}\n")
        ...
        with open(os.path.join(output_directory, 'log.txt'), 'a') as f:
            f.write("pubmed: AP50: {:.3f}, AP75: {:.3f}, AP: {:.3f}, AR: {:.3f}\n".
              format(pubmed_stats['coco_eval_bbox'][1],
                     pubmed_stats['coco_eval_bbox'][2],
                     pubmed_stats['coco_eval_bbox'][0],
                     pubmed_stats['coco_eval_bbox'][8]))

        if best_ap < pubmed_stats['coco_eval_bbox'][0]:
            # Save current model training progress if the accuracy beats the previous ones
            torch.save({'epoch': epoch,
                        'model_state_dict': model.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict(),
                        }, model_save_path)
            with open(os.path.join(output_directory, 'log.txt'), 'a') as f:
                f.write(f'==========Save best model at epoch{epoch}==========\n')
