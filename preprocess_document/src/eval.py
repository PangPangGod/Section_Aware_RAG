...
def eval_tsr_sample(target, pred_logits, pred_bboxes, mode):
    ...
    try:
        with open(img_words_filepath, 'r', encoding="utf-8") as f:
...
def visualize(args, target, pred_logits, pred_bboxes):
    ...
    bboxes_out_filename = img_filename.replace(".jpg", "_bboxes.jpg").replace(".png", "_bboxes.png")
    ...
    if args.data_type == 'structure':
        img_words_filepath = os.path.join(args.table_words_dir, img_filename.replace(".jpg", "_words.json").replace(".png", "_words.json"))
        cells_out_filename = img_filename.replace(".jpg", "_cells.jpg").replace(".png", "_cells.png")
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_every, header):
        ...
        if args.data_type == "structure":
            ...
            if batch_num % args.eval_step == 0 or batch_num == num_batches:
                ...
                for temp_i in range(len(targets_collection)):
                    metrics = eval_tsr_sample(targets_collection[temp_i], pred_bboxes_collection[temp_i], pred_bboxes_collection[temp_i], args.mode)
                tsr_metrics += [metrics]
                pred_logits_collection = []
                pred_bboxes_collection = []
                targets_collection = []
